...
class Storage_client(object):
    ...
    def _storage_do_upload_file(self, tracker_client, store_serv, \
        ...
        ret_dic = {
            ...
            'Remote file_id'  : group_name.strip(b'\x00') + __os_sep__.encode() + \
                                   remote_filename,
    ...
    def storage_delete_file(self, tracker_client, store_serv, remote_filename):
        ...
        try:
            ...
            send_buffer = struct.pack(del_fmt.encode(), store_serv.group_name, remote_filename.encode())
            tcp_send_data(store_conn, send_buffer)
            th.recv_header(store_conn)
            #if th.status == 2:
    ...
    def _storage_do_download_file(self, tracker_client, store_serv, file_buffer, \
        ...
        try:
            ...
            send_buffer = struct.pack(down_fmt.encode(), offset, download_size, \
                                      store_serv.group_name, remote_filename.encode())
            tcp_send_data(store_conn, send_buffer)
            th.recv_header(store_conn)
            #if th.status == 2:
            #    raise DataError('[-] Error: remote file %s is not exist.' % 
        ...
        ret_dic = {
            ...
            'Remote file_id' : store_serv.group_name + __os_sep__.encode() + remote_filename.encode(),
